#!/bin/bash
# Test generic tokenization framework capabilities
# This test validates the module's generic tokenization framework
# with callback support for enhanced field parsing.
...
template(name="tokenization_test_json" type="list" option.jsonf="on") {
    ...
    property(outname="tls_reason" name="$!win!TLSInspection!Reason" format="jsonf")
    property(outname="wdac_policy_version" name="$!win!WDAC!PolicyVersion" format="jsonf")
    property(outname="wdac_pid" name="$!win!WDAC!PID" format="jsonf")
    property(outname="wufb_policy_id" name="$!win!WUFB!PolicyID" format="jsonf")
...
action(type="omfile" file="'$RSYSLOG_OUT_LOG'" template="tokenization_test_json")
'

startup

# Test generic tokenization with complex field data
echo "Testing generic tokenization with complex field data..."
...
content_check '"wdac_policy_version":"3.2.0"' $RSYSLOG_OUT_LOG
content_check '"wdac_pid":"4128"' $RSYSLOG_OUT_LOG

# Validate TLS inspection fields
content_check '"tls_reason":"Unapproved Root Authority"' $RSYSLOG_OUT_LOG

# Validate WUFB fields
content_check '"wufb_policy_id":"2f9c4414-3f71-4f2b-9a7e-cc98a6d96970"' $RSYSLOG_OUT_LOG
