#!/bin/bash
# Test enhanced parsing capabilities with type awareness
# This test validates the module's enhanced parsing functions
# with type-aware value parsing and validation.
...
template(name="enhanced_parsing_json" type="list" option.jsonf="on") {
    property(outname="eventid" name="$!win!Event!EventID" format="jsonf")
    property(outname="logon_type" name="$!win!LogonInformation!LogonType" format="jsonf")
    property(outname="logon_type_name" name="$!win!LogonInformation!LogonTypeName" format="jsonf")
    property(outname="laps_policy_version" name="$!win!LAPS!PolicyVersion" format="jsonf")
    property(outname="laps_credential_rotation" name="$!win!LAPS!CredentialRotation" format="jsonf")
    property(outname="remote_credential_guard" name="$!win!Logon!RemoteCredentialGuard" format="jsonf")
...
# Validate TLS inspection fields
content_check '"eventid":"5157"' $RSYSLOG_OUT_LOG

# Validate WUFB fields
