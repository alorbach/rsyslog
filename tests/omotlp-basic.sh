...
# Function to generate HTTP configuration
generate_http_conf() {
    add_conf '
...
# Test HTTP protocol
echo "Testing OTLP HTTP protocol..."
generate_http_conf
startup
...
# Validate HTTP results
validate_otlp_results "http"

# Test gRPC protocol (Phase 2 - requires gRPC bridge)
echo "Testing OTLP gRPC protocol..."
# Note: gRPC testing would require Phase 2 implementation
# For now, we'll skip this and just clean up

# Function to validate OTLP results from collector output
validate_otlp_results() {
    local protocol=$1

    echo "Validating OTLP ${protocol} results..."
    ...
    if docker logs ${OTLP_CONTAINER_NAME} 2>/dev/null | grep -q "ResourceLogs"; then
        echo "✓ OTLP Collector received log data via ${protocol}"

        # Check file output
        if [ -f otlp-logs.json ]; then
            python3 -c "
import json
import sys

try:
    with open('otlp-logs.json', 'r') as f:
        # Read last few lines to get the most recent data
        lines = f.readlines()
        if not lines:
            print('ERROR: Empty OTLP logs file')
            sys.exit(1)
        ...
        if 'resourceLogs' not in data:
            print('ERROR: Missing resourceLogs in collector output')
            sys.exit(1)

        resource_logs = data['resourceLogs']
        if len(resource_logs) == 0:
            print('ERROR: Empty resourceLogs in collector output')
            sys.exit(1)
        ...
        print(f'✓ Found {found_records} log records in OTLP collector output')

        # Should have received some messages
        if found_records < 5:
            print(f'ERROR: Expected at least 5 log records, got {found_records}')
            sys.exit(1)

        print(f'✓ OTLP {protocol} validation passed')

except FileNotFoundError:
    print('ERROR: OTLP logs file not found')
    sys.exit(1)
except Exception as e:
    print(f'ERROR: Failed to validate OTLP data: {e}')
    sys.exit(1)
"
        else
            echo "ERROR: No OTLP logs file found"
            exit 1
        fi
    else
        echo "ERROR: No log data received by OTLP Collector"
        docker logs ${OTLP_CONTAINER_NAME}
        exit 1
    fi
}
