            ...
            for (size_t i = 0; i < len; ++i) {
                ...
                if (!json_object_is_type(entry, json_type_object)) {
                    LogError(0, RS_RET_INVALID_PARAMS,
                             "mmsnareparse: runtime config eventFields entry must be object");
                    continue;
                }
                event_field_mapping_t mapping;
                memset(&mapping, 0, sizeof(mapping));
                ...
                for (size_t j = 0; j < plen; ++j) {
                    ...
                    if (!json_object_is_type(patternObj, json_type_object)) {
                        LogError(0, RS_RET_INVALID_PARAMS,
                                 "mmsnareparse: runtime config pattern entry must be object");
                        continue;
                    }
...
static void parse_line(parse_context_t *ctx, char *line) {
    ...
    *colon = '\0';
              ...
              rest);
    {
        ...
        dbgprintf("[mmsnareparse DEBUG] parse_line: valueCopy='%s', calling handle_general_key\n",
