...
static const char *locate_snare_payload(const char *msg, smsg_t *pMsg) {
    ...
    if (pMsg != NULL) {
        ...
        if (tagBuf != NULL && tagLen > 0 && strstr((const char *)tagBuf, "MSWinEventLog") != NULL) {
            // MSWinEventLog is in tag - message body should be: version, channel, record, timestamp, eventid, provider...
            // But after syslog parsing, it might be: timestamp, eventid, provider... (version and channel missing)
            // Or it might be: version, channel, record, timestamp, eventid, provider...
            // Look for pattern: single digit "1" followed by tab/escaped-tab, then channel name
            const char *p = cursor;
            while (p != NULL && *p != '\0') {
                // Skip whitespace
                while (*p == ' ' || *p == '\t') p++;
                ...
                if (*p == '1' && (p[1] == '\t' || p[1] == ' ' || 
                    ...
                    if (strncmp(afterVersion, "Microsoft-Windows-", 19) == 0) {
                        ...
                        dbgprintf("[mmsnareparse DEBUG] locate_snare_payload: found MSWinEventLog in syslog tag, found version+channel at '%s'\n", p);
                        return p;
                    }
                }
                ...
                if (*p >= '0' && *p <= '9') {
                    ...
                    if (versionEnd > p && (*versionEnd == '\t' || *versionEnd == ' ' || 
                        (versionEnd[0] == '#' && versionEnd[1] == '0' && versionEnd[2] == '1' && versionEnd[3] == '1'))) {
            ...
            dbgprintf("[mmsnareparse DEBUG] locate_snare_payload: found MSWinEventLog in syslog tag, version+channel not found, using cursor '%s'\n", cursor);
            return cursor;
        }
    }
    ...
    if (cursor != NULL) {
        ...
        if (msWinEventLogPos != NULL) {
            ...
            if (msWinEventLogPos < cursor) {
                dbgprintf("[mmsnareparse DEBUG] locate_snare_payload: found MSWinEventLog before cursor, using RFC5424 format at '%s'\n",
                          msWinEventLogPos);
                ...
                dbgprintf("[mmsnareparse DEBUG] locate_snare_payload: found MSWinEventLog after cursor, using RFC5424 format at '%s'\n",
                          msWinEventLogPos);
                return msWinEventLogPos;
            }
        }
...
static const field_pattern_t *select_field_pattern(parse_context_t *ctx,
    ...
    if (sectionName == NULL && bestFallback != NULL) {
        ...
        for (size_t pass = 0; pass < 2; ++pass) {
            ...
            for (size_t i = 0; i < patternCount; ++i) {
                const field_pattern_t *candidate = &patterns[i];
                if (pattern_matches(candidate, label, canon) &&
                    candidate->section != NULL && strcmp(candidate->section, "EventData") == 0) {
                    return candidate;
                }
            }
        }
 ...
 */
static void populate_event_metadata(parse_context_t *ctx, char **tokens, size_t tokenCount, const char *rawMsg, smsg_t *pMsg) {
    ...
    if (isRFC5424) {
        ...
        if (tokenCount > 0 && !is_placeholder(tokens[0]) && strcmp(tokens[0], "MSWinEventLog") == 0) {
            ...
            size_t channelIdx = 1;
            if (tokenCount > 1 && tokens[0] != NULL && tokens[1] != NULL) {
                // Check if tokens[0] is version (single digit) and tokens[1] is channel (starts with "Microsoft-Windows-")
                if (tokens[0][0] >= '0' && tokens[0][0] <= '9' && tokens[0][1] == '\0' &&
                    strncmp(tokens[1], "Microsoft-Windows-", 19) == 0) {
                    channelIdx = 1;
                ...
                if (rawMsg != NULL) {
                    ...
                    if (msWinPos != NULL) {
                        ...
                        if (*afterMSWin == '\t' || *afterMSWin == ' ' ||
                            (afterMSWin[0] == '#' && afterMSWin[1] == '0' && 
                             afterMSWin[2] == '1' && afterMSWin[3] == '1')) {
                            // Skip tab/escaped-tab
                            if (*afterMSWin == '#') {
                                afterMSWin += 4;
                            } else {
                            ...
                            if (*afterMSWin == '\t' || *afterMSWin == ' ' ||
                                ...
                                while (*channelEnd != '\0' && *channelEnd != '\t' && *channelEnd != ' ' &&
                                       !(channelEnd[0] == '#' && channelEnd[1] == '0' && 
                                         channelEnd[2] == '1' && channelEnd[3] == '1')) {
                                    channelEnd++;
                                }
...
static rsRetVal parse_snare_text(
    ...
    if (tokenCount > descriptionIdx && !is_placeholder(tokens[descriptionIdx])) {
        ...
        if (fullDescription != NULL) {
            dbgprintf("[mmsnareparse DEBUG] parse_snare_text: calling parse_description with concatenated description (from token %zu): '%s'\n",
