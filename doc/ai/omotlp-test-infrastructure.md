...
### Method 1: Log Parsing (Primary)

The collector logs the bound port to stdout when using `0.0.0.0:0`. Patterns to look for:

- `"Listening on 0.0.0.0:4318"`
- `"HTTP receiver listening on [::]:4318"`
- `"Everything is ready. Begin running and processing data."` (indicates readiness)

The helper function extracts the port using:
```bash
grep -E "(Listening on|HTTP receiver listening on)" logfile | \
  grep -oE ":[0-9]+" | head -1 | sed 's/://'
```

### Method 2: lsof (Fallback)

If log parsing fails after 5 seconds, the helper uses `lsof`:

```bash
lsof -p $pid -a -iTCP -sTCP:LISTEN | grep -oE ":[0-9]+" | head -1 | sed 's/://'
```

### Method 3: Port File

After discovery, the port is written to `${RSYSLOG_DYNNAME}.otel_port.file` for test scripts to read:
